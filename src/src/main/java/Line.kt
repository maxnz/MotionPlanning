...
//    fun commonPoints(line: Line): Boolean {
//        if (p1.first.round(3) == line.p1.first.round(3) && p1.second.round(3) == line.p1.second.round(3) &&
//            p2.first.round(3) == line.p2.round(3)
//        )
//    }

    infix fun Pair<Double, Double>.eq(pair: Pair<Double, Double>): Boolean =
        first.round(3) == pair.first.round(3) && second.round(3) == pair.second.round(3)


infix fun intersects(line: Line) = when {
    this === line -> true
    p1 eq line.p1 && p2 eq line.p2 || p1 eq line.p2 && p2 eq line.p1 -> true
    (p1 on line || p2 on line || line.p1 on this || line.p2 on this) && slope == line.slope -> true
    p1 eq line.p1 || p1 eq line.p2 || p2 eq line.p1 || p2 eq line.p2 -> false
...
infix fun intersection(line: Line): Pair<Double, Double>? {
    return when {
        ...
        slope == null && line.slope == null -> throw Exception("Double null")
        slope == null -> {
            val x = p1.first
